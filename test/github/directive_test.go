...
func TestDirective(t *testing.T) {
	...
	b1 := balance.Get(ctx, gov.GovPublicAddress(cty.Organizer().Public), cty.MemberUser(0), qv.VotingCredits)
	if b1 != 10.0 {
		t.Errorf("expecting %v, got %v", 10.0, b1)
	}
	b2 := balance.Get(ctx, gov.GovPublicAddress(cty.Organizer().Public), cty.MemberUser(1), qv.VotingCredits)
	if b2 != 10.0 {
