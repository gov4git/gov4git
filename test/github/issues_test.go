...
func TestImportIssuesForPrioritization(t *testing.T) {
	...
	if ads1[0].ID.GitPath() != "github/issues/2" {
		t.Errorf("expecting github/issues/2, got %v", ads1[0].ID.GitPath())
	}
	if ads1[0].Closed || ads1[0].Frozen {
		t.Errorf("expecting open, not-frozen")
	}
	// issue-3: open, frozen
	if ads1[1].ID.GitPath() != "github/issues/3" {
		t.Errorf("expecting github/issues/3, got %v", ads1[1].ID.GitPath())
	}
	if ads1[1].Closed || !ads1[1].Frozen {
		t.Errorf("expecting open, frozen")
	}
	// issue-4: closed, frozen
	if ads1[2].ID.GitPath() != "github/issues/4" {
		t.Errorf("expecting github/issues/4, got %v", ads1[2].ID.GitPath())
	}
	...
	if ads2[0].ID.GitPath() != "github/issues/1" {
		t.Errorf("expecting github/issues/1, got %v", ads2[0].ID.GitPath())
	}
	if ads2[0].Closed || ads2[0].Frozen {
		t.Errorf("expecting open, not-frozen")
	}
	// issue-2: open, frozen
	if ads2[1].ID.GitPath() != "github/issues/2" {
		t.Errorf("expecting github/issues/2, got %v", ads2[1].ID.GitPath())
	}
	if ads2[1].Closed || !ads2[1].Frozen {
		t.Errorf("expecting open, frozen")
	}
	// issue-3: closed, frozen
	if ads2[2].ID.GitPath() != "github/issues/3" {
		t.Errorf("expecting github/issues/3, got %v", ads2[2].ID.GitPath())
	}
	if !ads2[2].Closed || !ads2[2].Frozen {
		t.Errorf("expecting closed, frozen")
	}
	// issue-4: open, frozen
	if ads2[3].ID.GitPath() != "github/issues/4" {
		t.Errorf("expecting github/issues/4, got %v", ads2[3].ID.GitPath())
	}
